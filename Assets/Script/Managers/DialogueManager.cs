...
public class DialogueManager : MonoBehaviour
{
    ...
    private DialogueSceneController dialogueSceneController;
    ...
    void Start()
    {
        ...
        dialogueSceneController = FindObjectOfType<DialogueSceneController>();
        if (dialogueSceneController != null)
        {
            Debug.Log("[DialogueManager] DialogueSceneController 발견 및 캐싱 완료");
        }
    }
    ...
    public void ShowCurrentDialogue()
    {
        ...
        if (currentData.HasBackgroundChange())
        {
            ...
            {
                cutsceneManager.ChangeBackground(backgroundName);
            }
        }
        ...
        string position = currentData.position.ToLower();
        bool showCharacter = (position != "empty");

        if (showCharacter)
        {
            // Position = Left/Right → 캐릭터 표시
            Debug.Log($"[DialogueManager] Showing character at {position}");
        ...
        {
            // Position = empty → 캐릭터만 숨김
            Debug.Log($"[DialogueManager] Hiding characters (Position=empty)");
            ...
            // 중요: 대화창은 유지! (자막 표시를 위해)
            // dialoguePanel.SetActive(false); ← 이걸 하지 않음!
        }
        ...
        if (typingEffect != null)
        {
            ...
            {
                typingEffect.StartTyping(cleanText);
            }
        }
    ...
    /// <summary>
    /// DialogueSceneController에게 대화 완료 알림
    /// </summary>
    private void NotifyDialogueComplete()
    {
        ...
        // DialogueSceneController가 있으면 호출
        if (dialogueSceneController != null)
        {
            Debug.Log("[DialogueManager] DialogueSceneController.OnDialogueComplete() 호출");
            dialogueSceneController.OnDialogueComplete();
        }
        else
        {
            // DialogueScene이 아닌 경우 (정상)
            Debug.Log("[DialogueManager] DialogueSceneController 없음 (DialogueScene이 아님)");
        }
    }
